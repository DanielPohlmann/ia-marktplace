---
name: dev-architecture-event-driven
description: Event-Driven Architecture (EDA), Event Sourcing, and CQRS -- complementary but independent patterns for building reactive, scalable systems with rich audit trails and temporal queries. USE FOR: EDA design (LinkDaily's domain-event → RabbitMQ integration-event two-step via MassTransit), CQRS command/query separation, event sourcing, projections, eventual consistency, compensating transactions, choreography vs orchestration, saga patterns DO NOT USE FOR: messaging channel topology and delivery guarantees (use dev-integration-patterns-messaging-channels), message routing rules (use dev-integration-patterns-message-routing), domain model design (use dev-architecture-domain-driven-design), MassTransit wiring (use stack-masstransit)
---
