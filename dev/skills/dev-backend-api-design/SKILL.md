...
description: Use when designing APIs — REST endpoints, GraphQL schemas, gRPC services, or WebSocket protocols — including resource naming, versioning, pagination, error handling, and API gateway patterns. USE FOR: REST API design for LinkDaily.Api endpoints (resource naming, HTTP semantics, versioning, pagination, error shapes, idempotency, OpenAPI specifications), GraphQL schema design, gRPC service definition, API gateway patterns DO NOT USE FOR: data storage design (use dev-backend-data-modeling), authentication mechanisms (use dev-backend-authentication), OpenAPI/Scalar UI wiring (use stack-openapi), API security hardening (use security-api-security)
---
