---
name: dev-architecture-domain-driven-design
description: Domain-Driven Design (DDD) strategic and tactical patterns based on Eric Evans' "Domain-Driven Design" -- covering bounded contexts, aggregates, context maps, and ubiquitous language for modeling complex domains. USE FOR: bounded context identification and context mapping (LinkDaily's Identity/Scheduling/Payments/Logistics/Notifications/Presentation/Reviews contexts), aggregate design, ubiquitous language enforcement, domain modeling, subdomain classification, tactical DDD patterns (entities, value objects, domain services, repositories) DO NOT USE FOR: event sourcing mechanics (use dev-architecture-event-driven), microservice decomposition (use dev-architecture-microservices), hexagonal ports/adapters (use dev-architecture-hexagonal), clean architecture layers (use dev-craftsmanship-clean-architecture)
---
