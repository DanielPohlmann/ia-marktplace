---
name: dev-backend-caching
description: Use when designing caching strategies and deciding which caching pattern fits a workload — cache-aside, read-through, write-through, write-behind, write-around, CDN, and HTTP caching. USE FOR: caching strategy selection, cache invalidation design, cache stampede prevention, multi-tier caching architecture, TTL and eviction policy choices, distributed cache topology decisions. DO NOT USE FOR: concrete Redis implementation details — commands, data structures, pub-sub, distributed locks (use stack-redis), database schema design (use dev-backend-data-modeling), API endpoint design (use dev-backend-api-design).
---
