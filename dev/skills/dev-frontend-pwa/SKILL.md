---
name: dev-frontend-pwa
description: Use when adding PWA capabilities to a web app — service worker lifecycle, caching strategies, offline support, web app manifest, push notifications, background sync, or Workbox configuration. USE FOR: installable web apps, service worker registration and caching (Cache-First, Network-First, Stale-While-Revalidate), Workbox setup, push notification delivery, background sync, HTTPS and manifest requirements DO NOT USE FOR: SPA routing and state management (use dev-frontend-spa), server-side rendering (use dev-frontend-ssr), frontend architecture selection (use dev-frontend), native mobile app development
---
...
> **Detailed how-tos with full code (service workers, caching strategies, manifest, installability, app shell, push, background sync, Workbox) live in [references/guide.md](references/guide.md); project-specific rules are in [references/rules.md](references/rules.md).** The material below is enough to orient and choose an approach.
