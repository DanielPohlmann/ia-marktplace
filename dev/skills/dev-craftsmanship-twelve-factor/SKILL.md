---
name: dev-craftsmanship-twelve-factor
description: Use when designing, deploying, or evaluating cloud-native applications against the Twelve-Factor App methodology (codebase, config, backing services, stateless processes, build/release/run, dev/prod parity, disposability, logs as streams). USE FOR: environment-based config for LinkDaily.Api and LinkDaily.Worker, backing service wiring (RabbitMQ, PostgreSQL, Redis as attached resources), stateless process design, graceful shutdown, dev/prod parity, log-as-event-stream setup DO NOT USE FOR: cloud infrastructure provisioning, container configuration (use tools-docker), microservice decomposition (use dev-architecture-microservices), specific library config (use stack-observability, stack-serilog)
---
