---
name: dev-craftsmanship-refactoring
description: Use when identifying code smells and applying systematic, behavior-preserving refactoring techniques — based on Martin Fowler's Refactoring. USE FOR: code smell catalog (bloaters, OO abusers, change preventers, dispensables, couplers), named refactoring technique selection (Extract Method, Introduce Parameter Object, Replace Conditional with Polymorphism, etc.), safe incremental transformation workflows, legacy code improvement. DO NOT USE FOR: readability and naming principles without smells present (use dev-craftsmanship-clean-code), design pattern selection (use dev-design-patterns), large-scale architecture restructuring (use dev-architecture).
---
