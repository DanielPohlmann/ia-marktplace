---
name: dev-backend-authentication
description: Use when designing authentication and authorization systems at the architecture level — OAuth 2.0 flows, JWT handling, session management, RBAC/ABAC models, multi-tenancy patterns, and security header configuration. USE FOR: authentication flow selection (Authorization Code + PKCE, Client Credentials, Device Code), JWT design and validation, session vs token trade-offs, RBAC/ABAC authorization models, multi-tenancy isolation, identity provider selection, security headers, CORS configuration. DO NOT USE FOR: security threat modeling and attack surface analysis (use security-authentication), ASP.NET Core Identity implementation with users/roles/claims in .NET (use stack-aspnet-identity), API endpoint design (use dev-backend-api-design).
---
