---
name: dev-frontend-micro-frontends
description: Use when splitting a frontend across independently deployed applications or teams via Module Federation, single-spa, import maps, iframes, or web components. USE FOR: micro-frontend architecture, runtime composition, Module Federation (Webpack 5/Rspack), single-spa shell apps, shared dependency management, cross-app state and event bus, independent frontend deployment pipelines DO NOT USE FOR: single-team SPA development (use dev-frontend-spa), server-side rendering (use dev-frontend-ssr), frontend architecture selection (use dev-frontend), backend service decomposition (use dev-architecture-microservices)
---
...
- [Micro-Frontends — Wikipedia](https://en.wikipedia.org/wiki/Microfrontend)
