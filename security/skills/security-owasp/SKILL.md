---
name: security-owasp
description: Use when applying OWASP standards to classify, assess, or baseline security vulnerabilities in LinkDaily.Api or any web/API surface. Covers OWASP Top 10 (2021), OWASP API Security Top 10 (2023), and CWE/SANS Top 25 with mitigation strategies for each category. USE FOR: OWASP Top 10 classification, API Security Top 10, CWE Top 25, vulnerability risk scoring, broken access control, injection, XSS, SSRF, security misconfiguration, insecure design DO NOT USE FOR: AI/LLM-specific adversarial vulnerabilities (use security-ai-security), automated security scanning tool setup in CI (use security-security-testing), cryptographic algorithm selection and implementation (use security-cryptography), injection rule authoring and allowlist validation (use security-input-validation)
---
