...
description: Use when hardening LinkDaily.Api endpoints or any REST API against common attack vectors. Covers rate limiting, CORS configuration, Content Security Policy, security headers, API gateway patterns, and API authentication strategies. USE FOR: API security controls, rate limiting, CORS configuration, CSP, security headers, API gateway patterns, API key management, OAuth for APIs, HTTPS enforcement, request validation DO NOT USE FOR: REST API design patterns and contract design (use dev-backend-api-design), OpenAPI/Swagger annotations and Scalar UI (use stack-openapi), authentication protocol design and OAuth2/OIDC flows (use security-authentication), ASP.NET Core Identity implementation (use stack-aspnet-identity)
---
