...
description: Use when selecting or implementing cryptographic controls for data protection in LinkDaily or any application. Covers TLS configuration, password hashing algorithms, key management, secrets management, and encryption standards across all platforms. USE FOR: TLS version and cipher suite selection, password hashing (Argon2/bcrypt/scrypt), symmetric and asymmetric encryption (AES/RSA/ECDSA), key management, secrets management (HashiCorp Vault, AWS KMS), certificate management, homomorphic encryption DO NOT USE FOR: authentication protocol design and OAuth2/OIDC flows (use security-authentication), ASP.NET Core Identity password hashing specifics (use stack-aspnet-identity), API security headers and CORS (use security-api-security), data masking and PII anonymization patterns (use security-data-protection), custom cryptographic algorithm design (use established libraries — never roll your own crypto)
---
...
- [NIST SP 800-175B Guide to Secure Web Services](https://csrc.nist.gov/publications/detail/sp/800-175b/rev-1/final)
