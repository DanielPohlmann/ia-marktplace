...
description: Use when implementing technical data protection controls for PII, PHI, or payment data — encryption at rest and in transit, data classification, masking, tokenization, retention policies, and privacy-by-design patterns. USE FOR: AES-256 field encryption, TLS enforcement, data classification tiers, PII masking, data minimization, GDPR/CCPA/HIPAA technical controls, key management, data retention DO NOT USE FOR: cryptographic algorithm selection (use security-cryptography), access control and identity design (use security-authentication), audit logging and SIEM (use security-logging-monitoring), legal rights and data-subject obligations (use legal-privacy-data-protection), .NET project hygiene (use stack-hygiene)
---
