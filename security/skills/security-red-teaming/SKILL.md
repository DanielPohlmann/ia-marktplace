...
description: Use when planning or conducting full-scope adversarial red team engagements that test an organization's detection, response, and resilience — going beyond vulnerability discovery to simulate realistic APT and insider-threat kill chains. Covers red team vs pen test distinctions, MITRE ATT&CK, Cyber Kill Chain, purple teaming, C2 frameworks, and AI/LLM red teaming. USE FOR: red teaming, adversary simulation, MITRE ATT&CK, Cyber Kill Chain, purple teaming, C2 frameworks, assumed breach, attack path mapping, detection gap testing, AI red teaming, LLM red teaming, tabletop exercises DO NOT USE FOR: automated vulnerability scanning in CI (use security-security-testing), initial vulnerability discovery scoped to specific targets (use security-penetration-testing), threat modeling during design phase (use security-threat-modeling), OWASP classification (use security-owasp)
---
