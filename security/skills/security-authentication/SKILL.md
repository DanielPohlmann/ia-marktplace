...
description: Use when designing or evaluating authentication and authorization systems for LinkDaily or any application. Covers OAuth 2.0, OpenID Connect, RBAC, ABAC, Zero Trust architecture, session management, and multi-factor authentication threats and mitigations. USE FOR: OAuth 2.0 flow selection, OpenID Connect/OIDC, RBAC/ABAC/ReBAC design, Zero Trust architecture, session management security, MFA threat analysis, JWT security, SSO, identity federation DO NOT USE FOR: ASP.NET Core Identity implementation for LinkDaily (use stack-aspnet-identity), cryptographic primitive selection and TLS configuration (use security-cryptography), API rate limiting and security headers (use security-api-security), general backend auth design patterns (use dev-backend-authentication)
---
