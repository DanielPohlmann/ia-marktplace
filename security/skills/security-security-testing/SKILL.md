...
description: Use when selecting and integrating security testing tools into CI/CD pipelines. Covers SAST, DAST, SCA, container scanning, secrets detection, and IaC security scanning. USE FOR: SAST, DAST, SCA, Semgrep, SonarQube, CodeQL, Snyk, OWASP ZAP, Burp Suite, Trivy, Falco, GitLeaks, TruffleHog, Checkov, container scanning, secrets scanning, IaC scanning. DO NOT USE FOR: dependency SBOM and artifact signing (use security-supply-chain), threat identification during design (use security-threat-modeling), security incident response and log monitoring (use security-logging-monitoring), OWASP Top 10 vulnerability remediation guidance (use security-owasp), functional or performance testing.
---
