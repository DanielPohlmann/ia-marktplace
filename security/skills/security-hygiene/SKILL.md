...
description: Use when enforcing defensive coding practices that treat all data as untrusted at every component boundary — including internal databases, caches, RabbitMQ message queues, and inter-service calls. Covers sanitization, canonicalization, output encoding, and zero-trust data handling across the full trust boundary model. USE FOR: data hygiene, zero-trust data handling, canonicalization, output encoding, internal data validation, defense in depth across component boundaries (HTTP, DB, cache, message queue) DO NOT USE FOR: injection attack patterns and allowlist rules (use security-input-validation), OWASP Top 10 classification (use security-owasp), cryptographic controls (use security-cryptography), API rate limiting and headers (use security-api-security), .NET project hygiene such as nullable references and build warnings (use stack-hygiene)
---
