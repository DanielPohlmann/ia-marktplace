---
name: tools-git
description: Git version control — workflows, branching, history manipulation, hooks, and configuration. Use when working with the LinkDaily Git workflow: lefthook hook setup/debugging, conventional commit formatting, branch strategy (feature → develop → main), conflict resolution, and advanced operations. USE FOR: branching strategies (Gitflow, trunk-based), merge vs rebase decisions, conflict resolution, cherry-pick, bisect, stash, worktrees, Git hooks (lefthook integration), .gitignore, git aliases, history rewriting (interactive rebase, fixup), git configuration. DO NOT USE FOR: GitHub/GitLab platform-specific features such as PR reviews or Actions pipelines (consult the relevant platform's own documentation), Git LFS administration, Docker container workflows (use tools-docker), CI/CD pipeline authoring (use tools-docker for the containerized build/pipeline side).
---
